...
const tokenJSON = require("../artifacts/contracts/Erc.sol/MerandToken.json")

describe("MerandShop", function (){
    let owner
    let buyer
    let shop
    let erc20
    ...
    beforeEach(async function(){
        ...
        const MerandShop = await ethers.getContractFactory("MerandShop", owner)
        shop = await MerandShop.deploy()
        await shop.deployed()
    ...
    it("should have an owner and a token", async function(){
        ...
        expect(await shop.token()).to.be.properAddress // проверка токена на адрес, токенам нужны адреса из wufflejs
    })
